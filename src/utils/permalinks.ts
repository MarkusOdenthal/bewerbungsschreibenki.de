...
import { APP_BLOG, SITE } from '~/site.config';
    ...
    for (const key in record) {
      if (key === 'href') {
        ...
        if (typeof value === 'string') {
          ...
          const hrefObj = value as Record<string, string>;
          if (hrefObj.type === 'home') {
            obj[key] = getHomePermalink();
          } else if (hrefObj.type === 'blog') {
            obj[key] = getBlogPermalink();
          } else if (hrefObj.type === 'asset') {
            obj[key] = getAsset(hrefObj.url);
          } else if (hrefObj.url) {
            obj[key] = getPermalink(hrefObj.url, hrefObj.type);
          }
        }
      } else {
        obj[key] = applyGetPermalinks(record[key] as Record<string, unknown>);
      }
    }
    return obj;
  }
